...
function buildAlarmFlex(alarm, subRows, trigger) {
    ...
    contents: [
      {
        ...
        backgroundColor: cfg.color, cornerRadius: "20px",
  ...
  bodyContents.push({
    ...
    contents: [{
      type: "text",
      text: "⚠️ ข้อมูลนี้เป็นแนวทางช่วยตัดสินใจเท่านั้น",
  ...
  const flexBody = {
    ...
    header: {
      ...
      backgroundColor: cfg.color, paddingAll: "14px",
      contents: [
  ...
  if (alarm.image_url?.startsWith("http")) {
    ...
    flexBody.hero = { type: "image", url: img, size: "full", aspectRatio: "20:9", aspectMode: "cover" };
  }
  ...
  return {
    ...
    contents: {
      ...
      header: {
        ...
        contents: [
          { type: "text", text: "● RA5IC · RAMATHIBODI", color: "#FFFFFF", size: "xxs" },
          { type: "text", text: emoji + " " + title, color: "#FFFFFF", size: "md", weight: "bold", margin: "xs" }
        ]
      },
      body: {
    ...
    contents: {
      ...
      header: {
        ...
        contents: [
          ...
          { type: "text", text: "หอผู้ป่วยวิกฤตศัลยกรรม", color: "#FFFFFF", size: "xs", margin: "xs" }
        ]
      },
      body: {
...
async function handleEvent(event) {
  ...
  if (message.type === "image") {
    ...
    touch(uid);
    await lineClient.replyMessage(replyToken, { type: "text", text: "🔍 กำลังวิเคราะห์ภาพ Alarm ด้วย AI...\nรอสักครู่ครับ ⏳" });
    try {
  ...
  await lineClient.replyMessage(replyToken, buildSubFlex(fbRows, "fallback"));
}
...
app.get("/", (_, res) => res.json({ status: "🏥 CRRT Bot RA5IC v3.0" }));

loadDB().then(() => {
  const PORT = process.env.PORT || 3000;
  app.listen(PORT, () => console.log(`✅ CRRT Bot v3.0 :${PORT}`));
});